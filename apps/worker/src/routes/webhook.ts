...
import type { WebhookRequestBody, WebhookEvent, TextEventMessage, GroupSource, RoomSource } from '@line-crm/line-sdk';
...
async function handleEvent(
  ...
  // ─── Group / Room targets (P0 group support) ──────────────────────────────
  // Group/room-sourced events are handled as conversation "targets" and never
  // fall through to the 1:1 friend paths. Auto-reply / scenario matching stays
  // friend-only: replying to every group utterance would spam multi-person
  // conversations, so groups only get registration + message logging here.
  if (event.source.type === 'group' || event.source.type === 'room') {
    await handleTargetEvent(db, lineClient, event, lineAccessToken, lineAccountId, workerUrl, r2);
    return;
  }
