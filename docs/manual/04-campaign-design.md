...
L Harnessの設計思想は、この欠落を埋めることに全振りしている。原則はたった一つ。
...
「GA4 で拾えばいい」は半分しか正しくない。LINE in-app browser は session として正しく扱えないケースがあり UTM が落ちる。仮に取れても**L Harness の friend_id と紐付ける手段がない**。「どの友だちがクリックしたか」は Harness の中でしか解けない問題で、外部解析ツールでは原理的に無理。
